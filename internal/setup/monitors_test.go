...
func TestRenderMonitorsOrdersLeftToRight(t *testing.T) {
	got := renderMonitors(workDesktop(), false)
...
func TestRenderMonitorsIsStableAcrossEnumerationOrder(t *testing.T) {
	...
	if a, b := renderMonitors(forward, false), renderMonitors(reversed, false); a != b {
...
// hypr-cachy, 2026-08-07: three 4K panels that each advertise 239.99Hz and
// cannot all be driven at it. Recording the best advertised mode wrote 239.99,
// Hyprland fell back, and the desk ran at 60 against a file claiming 240. The
// mode that is running is the only one known to work.
func hyprCachy() []Monitor {
	modes := []string{"3840x2160@239.99Hz", "3840x2160@120.00Hz", "3840x2160@60.00Hz"}
	return []Monitor{
		{Name: "DP-2", Description: "Dell AW3225QF BD52YZ3", Width: 3840, Height: 2160,
			RefreshRate: 120, X: 0, Y: 0, Scale: 2, AvailableModes: modes},
		{Name: "DP-1", Description: "Dell AW3225QF 4HF2YZ3", Width: 3840, Height: 2160,
			RefreshRate: 120, X: 1920, Y: 0, Scale: 2, AvailableModes: modes},
		{Name: "DP-3", Description: "Dell AW3225QF JDM1YZ3", Width: 3840, Height: 2160,
			RefreshRate: 120, X: 3840, Y: 0, Scale: 2, AvailableModes: modes},
	}
}

func TestRenderMonitorsRecordsTheRunningRateNotTheFastestAdvertised(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	got := renderMonitors(hyprCachy(), false)

	if strings.Contains(got, "239.99") {
		t.Errorf("recorded a rate the machine is not driving:\n%s", got)
	}
	if n := strings.Count(got, "3840x2160@120"); n != 3 {
		t.Errorf("got %d outputs at the running 120Hz, want 3:\n%s", n, got)
	}
}

// hyprctl reports the hardware's real rate; the mode list is in round numbers.
// Writing 59.997 back states a mode the panel never offered.
func TestRenderMonitorsSnapsToAnAdvertisedRate(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	got := renderMonitors(workDesktop(), false)
	if strings.Contains(got, "59.99") {
		t.Errorf("wrote the raw running rate instead of the advertised one:\n%s", got)
	}
	if n := strings.Count(got, "3840x2160@60"); n != 3 {
		t.Errorf("got %d outputs at 60Hz, want 3:\n%s", n, got)
	}
}

// An output running at a rate nothing in its mode list is near keeps that rate:
// it is what the machine is demonstrably doing, and inventing a nearby
// advertised one would be the same guess this file exists to avoid.
func TestRenderMonitorsKeepsAnUnlistedRate(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	mons := []Monitor{{Name: "DP-1", Width: 3840, Height: 2160, RefreshRate: 85,
		Scale: 1, AvailableModes: []string{"3840x2160@60.00Hz", "3840x2160@239.99Hz"}}}
	if got := renderMonitors(mons, false); !strings.Contains(got, "3840x2160@85") {
		t.Errorf("an unlisted running rate was not kept:\n%s", got)
	}
}

// -- scale is a preference ---------------------------------------------------

// The failure this guards: hyprland.lua was applied before monitors.lua
// existed, Hyprland auto-arranged everything at scale 1, and the capture wrote
// that back over a deliberate scale 2.
func TestRenderMonitorsNeverLowersARecordedScale(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, monitorsTarget), renderMonitors(hyprCachy(), false))
	...
	got := renderMonitors(flattened, false)
	if n := strings.Count(got, "scale = 2"); n != 3 {
		t.Errorf("a recorded scale 2 was lowered to the live 1:\n%s", got)
	}
}

func TestRenderMonitorsAcceptsALowerScaleWhenForced(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, monitorsTarget), renderMonitors(hyprCachy(), false))
	...
	got := renderMonitors(flattened, true)
	if strings.Contains(got, "scale = 2") {
		t.Errorf("--force did not accept the live scale:\n%s", got)
	}
	if n := strings.Count(got, "scale = 1"); n != 3 {
		t.Errorf("got %d outputs at the forced scale 1, want 3:\n%s", n, got)
	}
}

// Only a drop is held back. Raising a scale is something the user did on
// purpose in the compositor, and a re-capture is how they record it.
func TestRenderMonitorsAcceptsAHigherScale(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, monitorsTarget), renderMonitors(hyprCachy(), false))
	...
	if got := renderMonitors(raised, false); !strings.Contains(got, "scale = 3") {
		t.Errorf("a raised scale was not taken:\n%s", got)
	}
}

// A machine recording its layout for the first time has no floor to respect.
func TestRenderMonitorsWithNothingRecorded(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	if got := renderMonitors(hyprCachy(), false); !strings.Contains(got, "scale = 2") {
		t.Errorf("the live scale was not recorded:\n%s", got)
	}
}

func TestRecordedScalesIgnoresAnUnreadableFile(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	writeFile(t, filepath.Join(home, monitorsTarget), "this is not the generated format")
	if got := recordedScales(); len(got) != 0 {
		t.Errorf("recordedScales = %v, want nothing from a file it cannot parse", got)
	}
}

// -- the layout is never captured automatically ------------------------------

// The capture used to be a post-install step gated on the file's absence, which
// put it after `chezmoi apply` had already reset the display session. Nothing
// in the catalog may write this file.
func TestNoCatalogStepWritesTheLayout(t *testing.T) {
	for _, c := range catalog() {
		for _, s := range c.Post {
			if strings.Contains(strings.ToLower(s.Name), "monitor") ||
				strings.Contains(strings.ToLower(s.Name), "layout") {
				t.Errorf("component %q has post-install step %q — the layout must "+
					"only be recorded by `hydra monitors`", c.Key, s.Name)
			}
		}
	}
}
...
func TestLayoutCheck(t *testing.T) {
	...
	if layoutCheck().OK {
		t.Error("layoutCheck = OK with no layout recorded")
	}
	if fix := layoutCheck().Fix; !strings.Contains(fix, "hydra monitors") {
		t.Errorf("the fix should name the command that records it, got %q", fix)
	}

	writeFile(t, filepath.Join(home, monitorsTarget), renderMonitors(hyprCachy(), false))
	if !layoutCheck().OK {
		t.Error("layoutCheck = not OK with a layout on disk")
	}
}
